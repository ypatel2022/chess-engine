...
namespace {
void appendQuad(sf::VertexArray &vertices, const sf::Vector2f &topLeft,
...
uint64_t Board::removePiece(uint64_t piecePositions, int index,
                            PieceColor color) {
...
uint64_t Board::removePiece(uint64_t piecePositions, sf::Vector2i boardPos,
                            PieceColor color) {
  int index =
      getIndexFromPosition({(int)boardPos.x / 125, (int)boardPos.y / 125});
...
uint64_t Board::addPiece(uint64_t piecePositions, sf::Vector2i boardPos,
                         PieceColor color) {
  int index =
      getIndexFromPosition({(int)boardPos.x / 125, (int)boardPos.y / 125});
...
void Board::generatePseudoBishopMoves(int index, PieceColor color) {
  ...
  // look down+right
  // when encountering an 8 in the "x" pos, break out of loop to avoid
  // teleportation
...
void Board::highlightUserPosition(int index) {
  ...
  if (getBit(allBlackPieces, index) == 0 &&
      getBit(highlightedPossibleMoves, index) == 0) {
    return;
  }
...
void Board::processInput(const sf::Event &event) {
  ...
  int index =
      getIndexFromPosition({(int)pixelPos.x / 125, (int)pixelPos.y / 125});

  {
    ...
    if (const auto *pressed = event.getIf<sf::Event::MouseButtonPressed>()) {
      if (pressed->button == sf::Mouse::Button::Left && lock_click != true) {
        ...
        // highlightedPossibleMoves = addPiece(highlightedPossibleMoves, index,
        // PieceColor::White);
